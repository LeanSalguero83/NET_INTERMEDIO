...
namespace Data.Manager
{
	...
	{
		public RecuperarCuentaManager(ApplicationDbContext context) : base(context)
		{
		}
		...
		public Usuarios BuscarAsync(LoginDto usuario)
		{
			...
			{
				return context.Usuarios.Where(x => x.Codigo == usuario.Codigo).FirstOrDefault();
			}
			else
			{
				return context.Usuarios.Where(x => x.Mail == usuario.Mail).FirstOrDefault();
			}
